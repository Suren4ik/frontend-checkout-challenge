...
Фронтенд добавлен в `apps/web` (`@checkout/web`). API и фронтенд запускаются в двух терминалах:
...
- `npm run build:web` — проверка типов и сборка фронтенда;
- `npm run test:web` — unit-тесты фронтенда.

Устройство решения, разбор обработки данных, проверенные сценарии и известные недоработки описаны в [apps/web/README.md](apps/web/README.md).
